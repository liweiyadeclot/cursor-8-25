...
### 步骤 2：添加代码节点（Excel → 提示词）

**节点类型**：`代码`

**代码**（直接复制使用 - 内联版本，无需外部文件）：

⚠️ **重要**：由于 Dify 运行在服务器上，无法访问本地文件，请使用以下**完整内联版本**：

完整代码请参考：`Dify代码节点_简化版本.md`
...
# Excel 读取函数（内联）
def excel_to_json_inline(excel_path, sheet_name, serial):
    try:
        ...
        if project_number_idx is not None and rows:
            ...
            if project_number:
                json_data["project"] = {"projectNumber": project_number}
            ...
            for row in rows:
                ...
                if category_idx is not None and amount_idx is not None:
                    ...
                    if category and amount is not None:
                        expenses.append({"category": category, "amount": str(amount).strip()})
            if expenses:
        ...
        for row in rows:
            ...
            if person_id_idx is not None:
                ...
                if person_id:
                    personnel.append({"personId": person_id, "cardLastFour": card, "amount": amount})
        if personnel:
        ...
        if appointment_idx is not None and rows:
            ...
            if appointment is not None:
                json_data["appointment"] = {"date": str(appointment).strip()}
...
# MCP 提示词生成函数（内联）
def build_prompt_inline(json_data):
    segments = []
...
if not excel_path or not sheet_name or not serial:
    output = {"success": False, "error": "缺少必要参数"}
elif not os.path.exists(excel_path):
    output = {"success": False, "error": f"Excel 文件不存在: {excel_path}"}
else:
    try:
        json_data = excel_to_json_inline(excel_path, sheet_name, serial)
        if not json_data:
            output = {"success": False, "error": f"未找到序号 {serial} 的数据"}
        else:
            mcp_prompt = build_prompt_inline(json_data)
            if not mcp_prompt:
                output = {"success": False, "error": "未能生成有效的 MCP 提示词"}
            else:
                output = {"success": True, "mcp_prompt": mcp_prompt, "prompt_length": len(mcp_prompt)}
    except Exception as e:
        output = {"success": False, "error": f"处理失败: {str(e)}", "error_type": type(e).__name__}
```
...
# 输出结果（在 Dify 代码节点中，最后一行或 output 变量作为输出）
output = {
